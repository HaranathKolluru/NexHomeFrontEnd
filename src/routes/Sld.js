...
function Sld() {
  ...
  function handleDelete(id) {
    ...
    Axios.post(
      `https://nexhome-backend-project.onrender.com/nexHome/propertyDel/${currentUser._id}`,
      {
        id: id,
      }
    )
      ...
      .then(() => {
        Axios.delete(
          `https://nexhome-backend-project.onrender.com/sell/deletePropert/${id}`
        ).then((res) => {
          if (res.status === 200) {
            // alert("Successful");
  ...
  useEffect(() => {
    ...
    Axios.get(
      `https://nexhome-backend-project.onrender.com/nexHome/getUser/${currentUser._id}`
    ).then(async (res) => {

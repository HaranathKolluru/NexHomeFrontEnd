...
function Wishlist() {
  ...
  useEffect(() => {
    Axios.get(
      `https://nexhome-backend-project.onrender.com/nexHome/getWishList/${currentUser._id}`
    ).then(async (res) => {
  ...
  function handleWishDel(id) {
    ...
    Axios.patch(
      `https://nexhome-backend-project.onrender.com/nexHome/delWish/${currentUser._id}`,
      {
        id: id,
      }
    ).then(async (res) => {
